...
class SQL {
    ...
    /// 生成插入语句
    /// - Parameter model:
    static func insert(_ model: PKDBModel) -> (String, [DatabaseValueConvertible]) {
    ...
    /// - Parameter model:
    /// - Returns:
    static func update(_ model: PKDBModel) -> (String, [DatabaseValueConvertible]) {
    ...
    /// - Returns:
    private static func generateValues(_ defines: [ColumnDefine]) -> [DatabaseValueConvertible] {
        return defines.map({
            if let transform = $0.getPropertyValue?() as? ColumnTransformable, let value = transform.transformToColumnValue() {
